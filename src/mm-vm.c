...
{  
    ...
    if (prev && prev->rg_end == lower_bound) {
    ...
    if (prev->rg_next && upper_bound == prev->rg_next->rg_start) {
      prev->rg_end = prev->rg_next->rg_end;
      prev->rg_next = deleted->rg_next;
      free(deleted);      
    }

    // In danh sách vùng nhớ trống (tùy thuộc vào hàm print_list_rg)
